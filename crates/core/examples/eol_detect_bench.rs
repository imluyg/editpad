...
//!   ＝**行为决策，等用户点单**，不是机械活。
//!
//! **第 216 轮（P306）改了那道等点单的：按余量早停**（`EolTally::is_decisive`，
//! 用户点单批准）。于是上面那几列的含义变了，别照原样读：
//! * "逐字符"那一列现在是**早停后**的数——56MB 样本 42.5 ms → **0.2 ms**；
//! * "按字节"原型没有早停，反而更慢（0.4×）⇒ 这一列现在的含义是
//!   "光换迭代方式、不改算法，没有意义"，不是"按字节变慢了"；
//! * 真实开档（分母）随之 208~214 ms → **157~161 ms ≈ 425 MB/s**
//!   （debug 2.5~2.75 s → 1.95~1.98 s）。
//!
