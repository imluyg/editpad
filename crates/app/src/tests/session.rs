...
fn closing_named_tab_writes_back_cursor_memory_to_disk() {
    ...
    assert_eq!(
        persisted.recent_view(Path::new("C:/b.txt")).map(|v| v.line),
        Some(2)
    );
...
fn snapshot_exit_writes_session_and_closes_without_confirm() {
    ...
    dispatch(
        &mut app,
        Message::Edit(EditOp::InsertText("草稿内容\r\n第二行".into())),
    );
    ...
    // 清单完整落盘且可解析
    let manifest =
        editpad_core::snapshot::read_manifest(&dir).expect("关窗后必须有可解析的会话清单");
...
fn snapshot_exit_captures_cursor_and_scroll_positions() {
    ...
    dispatch(
        &mut app,
        Message::Edit(EditOp::InsertText("a\nb\nc\n".into())),
    );
    {
...
fn snapshot_failure_degrades_to_confirm_bar_with_status_reason() {
    ...
    dispatch(
        &mut app,
        Message::Edit(EditOp::InsertText(" precious ".into())),
    );
    let _ = app.handle_close_request(iced::window::Id::unique(), Some(dir.clone()));
...
fn discard_and_close_clears_snapshot_area() {
    ...
    dispatch(
        &mut app,
        Message::Edit(EditOp::InsertText("will discard".into())),
    );
    ...
    dispatch(
        &mut app2,
        Message::Edit(EditOp::InsertText("new session".into())),
    );
...
fn session_restore_rebuilds_tabs_content_active_and_pending_loads() {
    ...
    assert_eq!(
        (ed0.cursor.line, ed0.cursor.col),
        (1, 2),
        "光标回到退出时位置"
    );
    drop(ed0);
...
fn remember_session_disabled_means_blank_start_and_ask_style_close() {
    ...
    assert!(
        !session_restore_allowed(true, false),
        "关闭会话记忆 = 不恢复也不写清单"
    );
    ...
    dispatch(
        &mut app,
        Message::Edit(EditOp::InsertText("keep me".into())),
    );
    let _ = app.handle_close_request(iced::window::Id::unique(), Some(dir.clone()));
...
fn restore_chain_resumes_after_an_unrelated_load_settles() {
    ...
    dispatch(
        &mut app,
        Message::FileDropped(PathBuf::from("C:/w/user.txt")),
    );
...
fn restore_skips_unloadable_page_and_keeps_chain_going() {
    ...
    dispatch(
        &mut app,
        Message::Loaded(seq_a, Err("文件不存在".to_owned())),
    );
...
fn loaded_language_routes_through_alias_and_sniff() {
    ...
    dispatch(
        &mut app2,
        Message::FileDropped(PathBuf::from("C:/bin/build")),
    );
...
fn p102_window_geometry_events_update_and_persist_settings() {
    ...
    dispatch(
        &mut app,
        Message::WindowMoved(iced::Point::new(320.0, 180.0)),
    );
    ...
    dispatch(
        &mut app,
        Message::WindowMoved(iced::Point::new(333.0, 188.0)),
    );
...
fn p63_late_skip_after_own_manual_save_is_silently_dropped() {
    ...
    dispatch(
        &mut app,
        Message::TabAutosaved(tid, v, path.clone(), AutosaveOutcome::SkippedExternalChange),
    );
    assert!(
...
fn window_focus_prompts_for_dirty_tab_and_respects_decision() {
    ...
    dispatch(
        &mut app,
        Message::Edit(EditOp::InsertText("local edit".into())),
    );
