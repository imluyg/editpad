...
impl EditorCore {
    ...
    pub(crate) fn line_text_ref(&self, line: usize) -> Rc<str> {
        // P292：额度按"本帧要画的行"给，不按一条拍脑袋的小数给。原值 256 KB 是
        // 按短行夹具定的，而本仓的主用例（单行日志 / 压缩文件 / 长 CSV）一行就
        // 能有几万字符——14 个可见行 × 20000 字符＝280 KB 已经越过旧额度，越过之后
        // 的策略是**拒绝入表**，于是同一行又被正文/选区/行布局各抄一遍，P283 那条
        // "每行一帧只物化一次"在长行下静默失效（实测：短行 17 次 / 20k 字符 21 次 /
        // 60k 字符 39 次，后者已退回 P283 改前的 47 次量级）。
        // 2 MB 是这个夹具的上一档量级（约 34 行 × 60000 字符），且它仍是**硬上限**：
        // 每个页签一份、随内容纪元清空，与文档总大小无关。
        const LINE_TEXT_MEMO_MAX_BYTES: usize = 2 * 1024 * 1024;
