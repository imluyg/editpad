...
fn close_window_during_active_load_degrades_safely() {
    ...
    dispatch(
        &mut app,
        Message::FileDropped(PathBuf::from("C:/loading.txt")),
    );
    ...
    dispatch(
        &mut app2,
        Message::FileDropped(PathBuf::from("C:/loading2.txt")),
    );
    ...
    dispatch(
        &mut app3,
        Message::FileDropped(PathBuf::from("C:/loading3.txt")),
    );
...
fn loaded_routes_to_moved_tab_after_earlier_tab_closed() {
    ...
    dispatch(
        &mut app,
        Message::FileDropped(PathBuf::from("C:/moved.txt")),
    );
    assert!(app.active_load.is_some());
    let seq = app.job_seq;
