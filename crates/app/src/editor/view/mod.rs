...
impl EditorView {
    ...
    /// S-5 第五步：查找命中底色自 `draw` 提成方法（A 层，函数体逐字搬移）。
    ///
    /// 与列块同款走显式传参：`DrawFrame` 在本块之后才构造得出来。`vis_first`／
    /// `vis_last` 由调用方算好传入（`visible_range()` 的结果在 `draw` 里被多处共用，
    /// 方法内再算一次等于把同一帧的量取两遍）。
    ///
    /// 护栏：开态 `wrap_find_highlight_paints_on_the_hit_visual_segment`＋关态
    /// `find_highlight_paints_only_the_hit_span_when_wrap_off`。⚠️ 关态那条是
    /// **第 162 轮现补的**：本块有开态／关态两份拆段循环，第①步实测把整块短路时
    /// 全量只有开态一条红——关态分支此前无人看守（O-1 那条断的是取串次数上界，
    /// 短路只会让次数更小，恒不红）。
    #[allow(clippy::too_many_arguments)]
    fn draw_find_hits(
        &self,
...
impl Widget<crate::Message, Theme, iced::Renderer> for EditorView {
    ...
    fn draw(
        ...
        // 查找命中底色（P123，S-5 第五步外提为 `draw_find_hits`，逐字搬移）。
        // 护栏：开态／关态各一条两帧差分（见 `draw_find_hits` 文档注释）。
        self.draw_find_hits(
            renderer,
            &core,
            bounds,
