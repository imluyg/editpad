...
fn new_tab_switch_and_pages_stay_independent() {
    ...
    assert!(app.cur_handle.borrow().doc.is_empty(), "新页应为空文档");
...
fn close_tab_flow_respects_dirty_and_never_empties_tabs() {
    ...
    dispatch(
        &mut app,
        Message::Edit(EditOp::InsertText("dirty page".into())),
    );
...
fn window_close_confirms_when_any_background_tab_is_dirty() {
    ...
    dispatch(
        &mut app,
        Message::Edit(EditOp::InsertText("make dirty".into())),
    );
    ...
    assert!(app.confirm_visible, "任一页置脏都必须弹关窗确认");
}
...
fn ctx_menu_card_h_adapts_to_viewport() {
    ...
    let (_, ay) = clamp_menu_anchor(
        (100.0, 500.0),
        (800.0, 120.0),
        CTX_MENU_W,
        ctx_menu_card_h(120.0),
    );
    assert!(
        ay + ctx_menu_card_h(120.0) <= 120.0,
        "锚点+高度必须落在窗内"
    );
}
...
fn context_menu_anchor_recorded_from_cursor_and_viewport_tracked() {
    ...
    dispatch(
        &mut app,
        Message::CursorMoved(iced::Point::new(300.0, 200.0)),
    );
...
fn view_builds_with_floating_overlays_open() {
    ...
    dispatch(&mut app, Message::TabContextMenu(0));
    let is_menu_card = |b: iced::Rectangle| (195.0..=215.0).contains(&b.width) && b.height > 200.0;
...
fn close_other_tabs_aggregates_dirty_confirm_and_spares_pinned() {
    ...
    assert!(
        app.tabs[0].pinned && app.tabs[0].dirty,
        "固定置脏页原样幸存"
    );
...
fn p65_double_click_detector_contract() {
    ...
    // 同页但超窗（+1ms）：不算
    let late = t0
        + std::time::Duration::from_millis(TAB_DOUBLE_CLICK_MS)
...
fn tab_rename_inline_renames_file_and_migrates_recents() {
    ...
    dispatch(
        &mut app,
        Message::TabRenameInputChanged("whatever.txt".into()),
    );
...
fn rename_commit_targets_the_page_it_started_on_not_the_index() {
    ...
    dispatch(
        &mut app,
        Message::TabRenameInputChanged("renamed-b.txt".into()),
    );
