...
fn ime_events_are_gated_by_focus() {
    ...
    assert_eq!(
        c.ime_commit("你好"),
        ImeCommit::Consumed(Some("你好".into()))
    );
...
#[test]
fn enter_inherits_leading_indent_of_current_line() {
    let mut c = core_with("    foo\nbar");
    c.cursor = CursorPos { line: 0, col: 7 }; // 行尾
    c.enter();
    assert_eq!(c.doc.to_text(), "    foo\n    \nbar");
    assert_eq!(c.cursor, CursorPos { line: 1, col: 4 }, "光标落到缩进之后");
}

#[test]
fn enter_inside_indent_stops_at_caret() {
    // 光标停在行首空白内：只带到光标处，不把其后的空白拖到新行
...
#[test]
fn enter_on_whitespace_only_line_keeps_up_to_caret() {
    // 纯空白行：行首空白整体继承（首个非空白不存在，取光标列兜底）
    let mut c = core_with("\t \nend");
    c.cursor = CursorPos { line: 0, col: 2 };
    c.enter();
    assert_eq!(c.doc.to_text(), "\t \n\t \nend");
...
#[test]
fn enter_uses_dominant_eol_and_is_undoable() {
    // CRLF 文档：换行归一为 \r\n；换行+缩进一次插入、可整体撤销
    let mut c = core_with("  a\r\nb");
    c.cursor = CursorPos { line: 0, col: 3 };
    c.enter();
