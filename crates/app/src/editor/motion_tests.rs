...
fn cjk_mono_candidates_pick_by_priority_with_loose_family_matching() {
    ...
    let installed = [
        "Noto Sans Mono CJK SC".to_owned(),
...
fn scrollbar_alpha_fades_after_idle_window() {
    ...
    assert_eq!(scrollbar_alpha(SCROLLBAR_IDLE_MS + SCROLLBAR_FADE_MS), 0.0);
    // 之后恒 0（不回弹）
    assert_eq!(
...
fn scrollbar_visibility_touch_fade_and_refade() {
    ...
    c.sb_activity = Some(
        t0 - std::time::Duration::from_millis((SCROLLBAR_IDLE_MS + SCROLLBAR_FADE_MS / 2) as u64),
    );
    ...
    c.sb_activity = Some(
        t0 - std::time::Duration::from_millis((SCROLLBAR_IDLE_MS + SCROLLBAR_FADE_MS + 50) as u64),
    );
    assert_eq!(c.scrollbar_visibility_at(t0), 0.0);
    assert!(!c.scrollbar_fading_at(t0));
...
fn measure_char_advance(font: Font, size: f32, ch: char) -> Option<f32> {
    ...
    measure_text_advance(font, size, &ch.to_string().repeat(8)).map(|w| w / 8.0)
}
...
fn injected_row_layout_matches_real_shaping() {
    ...
    for text in lines {
        ...
        assert_eq!(xs.len(), chars.len() + 1, "行 {text:?} 布局长度 = 字符数+1");
...
fn typing_beyond_viewport_scrolls_immediately_despite_stale_layout() {
    ...
    let expect_scroll = ((405.0 - view_w + CARET_WIDTH).max(0.0)).min((405.0 - view_w).max(0.0));
    assert!(
...
fn hscrollbar_threshold_matches_real_content_width() {
    ...
    assert!(!HScrollbar::measure(c.content_width_px(), view, 800.0, 0.0).needed);
...
fn hscrollbar_geometry_mirrors_vertical_contract() {
    ...
    for &scroll in &[0.0f32, 100.0, 777.7] {
        let back =
            sb.scroll_for_thumb_x(sb.thumb_x + scroll / sb.range_px * (sb.track_w - sb.thumb_w));
        assert!(
...
fn ensure_visible_follows_cursor_horizontally() {
    ...
    c.ensure_visible_pub();
    assert!(
        (c.scroll_left - mid).abs() < f32::EPSILON.max(mid * 1e-6),
        "可见范围内的光标不得扰动横向滚动"
    );
}
...
fn scroll_by_columns_clamps_to_content_width() {
    ...
    let max_expected = (c.max_line_display_cols() as f32 * c.char_width()) - c.text_viewport_w();
    assert!(
        (c.scroll_left - max_expected).abs() < 1.0,
        "右缘应贴住最宽行"
    );
}
...
fn hit_test_accounts_for_horizontal_scroll_offset() {
    ...
    let hit_scrolled = c.hit_test(gutter + (target_col as f32 - 30.0) * char_w + 1.0, 10.0);
    assert_eq!(hit_unscrolled.col, target_col);
    assert_eq!(
        hit_scrolled.col, target_col,
        "横滚后命中测试必须补偿 scroll_left"
    );
}
...
fn restore_view_places_cursor_and_scroll_without_side_effects() {
    ...
    assert!(
        (wide.scroll_top - 20.0).abs() < 1e-4,
        "行程内的纵向滚动应原样保留，实际 {}",
        wide.scroll_top
    );
    assert!(
        (wide.scroll_left - 40.0).abs() < 1e-4,
        "行程内的横向滚动应原样保留，实际 {}",
        wide.scroll_left
    );
}
...
fn thumb_drag_roundtrips_through_inverse_mapping() {
    ...
    assert_eq!(
        sb.scroll_for_thumb_y(sb.track_y - 50.0),
        0.0,
        "拖过头=回到顶部"
    );
...
fn track_click_centers_thumb_on_cursor() {
    ...
    assert!(
        (center_after - click).abs() < 1.5,
        "点击 {click}，中心停在 {center_after}"
    );
}
...
fn caret_blink_phase_toggles_and_activity_forces_visible() {
    ...
    c.tick_blink_at(t0 + std::time::Duration::from_millis(CARET_BLINK_MS + 33));
...
fn bracket_cache_invalidates_on_undo_redo_and_doc_replacement() {
    ...
    assert_eq!(d.bracket_match(), None, "静默重载后不得吐旧文档的陈旧配对");
...
#[test]
fn word_left_skips_ws_then_word_run() {
    let mut c = core_with("foo   bar");
    c.cursor = CursorPos { line: 0, col: 9 };
    c.apply_motion(Motion::WordLeft, false);
    assert_eq!(c.cursor, CursorPos { line: 0, col: 6 }, "先落词首 bar");
    c.apply_motion(Motion::WordLeft, false);
    assert_eq!(
        c.cursor,
...
fn word_motion_crosses_line_boundaries() {
    ...
    assert_eq!(
        c.cursor,
        CursorPos { line: 0, col: 2 },
        "行首再往左 = 上一行行尾"
    );
    c.apply_motion(Motion::WordRight, false);
    assert_eq!(
        c.cursor,
        CursorPos { line: 1, col: 0 },
        "行尾再往右 = 下一行行首"
    );
    c.apply_motion(Motion::WordRight, false);
    assert_eq!(
...
#[test]
fn word_motion_steps_single_punctuation() {
    let mut c = core_with("ab.cd");
    c.cursor = CursorPos { line: 0, col: 5 };
    c.apply_motion(Motion::WordLeft, false);
    assert_eq!(c.cursor, CursorPos { line: 0, col: 3 });
    c.apply_motion(Motion::WordLeft, false);
    assert_eq!(c.cursor, CursorPos { line: 0, col: 2 }, "单个标点一步");
    c.apply_motion(Motion::WordLeft, false);
    assert_eq!(c.cursor, CursorPos { line: 0, col: 0 });
}

#[test]
fn word_motion_treats_cjk_run_as_one_word() {
    let mut c = core_with("你好世界 abc");
    c.cursor = CursorPos { line: 0, col: 8 }; // 行尾
    c.apply_motion(Motion::WordLeft, false);
    assert_eq!(c.cursor, CursorPos { line: 0, col: 5 }, "先落 abc 词首");
    c.apply_motion(Motion::WordLeft, false);
    assert_eq!(
        c.cursor,
        CursorPos { line: 0, col: 0 },
...
#[test]
fn word_motion_at_document_edge_is_noop() {
    let mut c = core_with("ab");
    c.cursor = CursorPos { line: 0, col: 0 };
    c.apply_motion(Motion::WordLeft, false);
    assert_eq!(c.cursor, CursorPos { line: 0, col: 0 });
